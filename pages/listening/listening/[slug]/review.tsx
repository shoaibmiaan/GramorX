...
export default function ListeningReviewPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      (sRes.data ?? []).forEach(s => { tmap[s.order_no] = s.transcript ?? ''; });
      setTranscripts(tmap);
  ...
  const highlight = (text: string, needle: string, cls: string) => {
    ...
    return (
      <>
        {parts.map((part, i) =>
          re.test(part) ? (
            <span key={i} className={`rounded-ds px-1 ${cls}`}>{part}</span>
          ) : (
  ...
  const renderTranscript = (raw: string, correct: string, user: string) => {
    ...
    // First highlight correct answer (success)
    let node = highlight(raw, correct, 'bg-success/15 text-success');
    // If user different, add a second pass (warning). This is a simple pass; overlap is okay.
    if (!ok && user) {
      node = (
        <span className="contents">
          {highlight(
            // @ts-ignore: casting JSX back to string not needed; we re-run on raw for simplicity
            raw,
            user,
            'bg-sunsetOrange/20 text-sunsetOrange'
          )}
        </span>
      );
      // Render both, with a legend
      return (
        <div className="space-y-2">
          <div className="text-small opacity-80">
            <span className="rounded-ds px-1 bg-success/15 text-success mr-2">Correct</span>
            {!ok && <span className="rounded-ds px-1 bg-sunsetOrange/20 text-sunsetOrange">Your answer</span>}
          </div>
          <p className="opacity-90">{highlight(raw, correct, 'bg-success/15 text-success')}</p>
          {!ok && <p className="opacity-90">{highlight(raw, user, 'bg-sunsetOrange/20 text-sunsetOrange')}</p>}
        </div>
      );
    }
    return <p className="opacity-90">{node}</p>;
  };
    ...
    <section className="py-24 bg-lightBg dark:bg-gradient-to-br dark:from-dark/80 dark:to-darker/90">
      ...
      <style jsx global>{`
        @media print {
          ...
          #print-area { position: absolute; inset: 0; padding: 0 16px; }
      ...
      <Container id="print-area" ref={printRef}>
        <div className="flex items-center justify-between gap-4 no-print">
          ...
          <div className="flex gap-3">
            ...
            <Button variant="secondary" as="a" href={`/listening/${slug}`}>Retake</Button>
            <Button variant="secondary" as="a" href="/listening">Back</Button>
          </div>
        </div>
        ...
        <Card className="p-6 mt-8">
          <div className="flex flex-wrap items-center gap-4">
            <Badge variant="success">Correct: {result.correct} / {result.total}</Badge>
            ...
            return (
              <Card key={q.id} className="p-6">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <Badge variant={ok ? 'success' : 'danger'} size="sm">{ok ? 'Correct' : 'Incorrect'}</Badge>
                </div>
                ...
                {q.type === 'mcq' ? (
                    ...
                    {(q.options ?? []).map(opt => {
                      ...
                      return (
                        <li key={opt} className={`p-3.5 rounded-ds border ${cls}`}>
                          ...
                          {isChosen && !isCorrect && <i className="fas fa-times-circle ml-2 text-sunsetOrange" aria-hidden />}
                        </li>
                      );
                    })}
                  </ul>
                  ...
                  <div className="mt-4 space-y-2">
                    {ok ? (
                      <Alert variant="success"><strong>Your answer:</strong> {userAns}</Alert>
                    ) : (
                      <>
                        <Alert variant="error"><strong>Your answer:</strong> {userAns || <em>(blank)</em>}</Alert>
                        <Alert variant="info"><strong>Correct:</strong> {q.answer}</Alert>
                      </>
                    )}
                  </div>
                )}
                ...
                {compare && (
                  <div className="mt-5">
                    <div className="text-small font-semibold mb-1 opacity-80">Transcript excerpt (Section {q.sectionOrder})</div>
                    <Card className="p-4">
