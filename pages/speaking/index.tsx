...
type AttemptRow = {
  ...
  created_at: string;
  overall?: number | null;
  parts?: Record<'p1' | 'p2' | 'p3', number | null>;
};
      ...
      <Container>
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-grayish mt-2">Simulator, AI partner, roleplay, and detailed reports.</p>
          </div>
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          ...
          <Card className="card-surface p-6 rounded-ds-2xl">
            ...
            <p className="text-grayish mt-1">Parts 1–3 with prompts and timing.</p>
            <Button
              variant="primary"
          ...
          <Card className="card-surface p-6 rounded-ds-2xl">
            ...
            <p className="text-grayish mt-1">Practice answers with live feedback.</p>
            <Button
              variant="secondary"
              className="mt-6 rounded-ds-xl"
          ...
          <Card className="card-surface p-6 rounded-ds-2xl">
            ...
            <h3 className="text-h3 mt-3">Roleplay</h3>
            <p className="text-grayish mt-1">Real-life dialogues (UK/US/AUS).</p>
            <Button
              variant="secondary"
              className="mt-6 rounded-ds-xl"
              aria-label="Try Roleplay Check-in"
          ...
          {/* Recent attempts (only if signed in) */}
          {signedIn && (
  ...
  if (atts && atts.length) {
    ...
    if (clips) {
      ...
      clips.forEach((c: any) => {
        ...
        if (!row) return;
        row.parts![c.part as 'p1' | 'p2' | 'p3'] = Number(c.overall);
        (sums[c.attempt_id] ??= []).push(Number(c.overall));
      });
      Object.entries(sums).forEach(([id, arr]) => {
        const avg = arr.reduce((a, b) => a + b, 0) / arr.length;
        map.get(id)!.overall = Math.round(avg * 10) / 10;
      });
    }
